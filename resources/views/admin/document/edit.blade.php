		...
		<div class="box box-block bg-white">
			...
			<form class="form-horizontal" action="{{route('admin.document.update', $document->id )}}" method="POST" enctype="multipart/form-data" role="form">
				...
				<div class="form-group row">
					...
					<div class="col-xs-10">
						<select name="type">
							<option value="DRIVER" selected="{{$document->shown_to === 'DRIVER'}}">@lang('admin.document.driver_review')</option>
							<option value="VEHICLE" selected="{{$document->shown_to === 'VEHICLE'}}">@lang('admin.document.vehicle_review')</option>
						</select>
					</div>
				</div>

				<div class="form-group row">
					<label for="name" class="col-xs-2 col-form-label">@lang('admin.shown_to')</label>
					<div class="col-xs-10">
						<select name="shown_to">
							<option value="Motoboy" selected="{{$document->shown_to === 'Motoboy'}}">@lang('admin.document.motoboy')</option>
							<option value="Bicicleta" selected="{{$document->shown_to === 'Bicicleta'}}">@lang('admin.document.bike')</option>
						</select>
					</div>
				</div>
