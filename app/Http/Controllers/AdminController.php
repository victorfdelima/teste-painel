...
class AdminController extends Controller {
        ...
        try {
            ...
            if($request->has('city')){                
              ...
              $totalRides = UserRequests::whereHas('user', function ($query) use ($request) {
				 ...
				 $km = UserRequests::whereHas('user', function ($query) use ($request) {
                    $query->where('city_id', $request->get('city'));
            ...
            $scheduled_rides =UserRequestPayment::count();
            $fleet = Fleet::count();            
			...
			$km1 = UserRequestPayment::select(\DB::raw('SUM(distance + distance ) as distancia' ));
	
            $km1 = $km1->get();

            return view('admin.dashboard', compact('providers','km' ,'km1','fleet', 'provider', 'scheduled_rides', 'users', 'fleetCities', 'rides', 'totalRides', 'user_cancelled', 'provider_cancelled', 'cancel_rides', 'revenue', 'wallet'));
        } catch (Exception $e) {
        ...
        if (!file_exists($config)) {
            ...
            $data = "<?php 
				...
				return array(
					...
					'sos_number' => '190',
					'contact_number' => '',
					'contact_email' => 'contato@urbis-cafelandia.com.br',
					'environment' => '',
					'ios_push_password' => '',
					'android_push_key' => '',
    ...
    public function settings_payment_store(Request $request) {
        if ($request->has('card')) {
            $this->validate($request, [
                ...
                //TODO MARCIO-HAMASAKI - Alterações PIC_PAY
                'debit_machine' => 'in:on',
                'pic_pay' => 'in:on',
				'mercado_pago' => 'in:on',
                'payumoney' => 'in:on',
        ...
        if (!$request->has('daily_target')) {
            ...
            if (!$request->has('mercado_pago')) {
                $search_text = "'mercado_pago' => '1'";
                $value_text = "'mercado_pago' => '0'";
                $change_content = str_replace($search_text, $value_text, $change_content);
            }
			
			if (!$request->has('pic_pay')) {
                $search_text = "'pic_pay' => '1'";
                $value_text = "'pic_pay' => '0'";
                $change_content = str_replace($search_text, $value_text, $change_content);
            }
            ...
            //TODO MARCIO-HAMASAKI - Alterações PIC_PAY
            if (($request->has('cash') == 0 && $request->has('card') == 0) && $request->has('payumoney') == 0 && $request->has('pic_pay') == 0 && $request->has('debit_machine') == 0 && $request->has('paypal') == 0 && $request->has('paypal_adaptive') == 0 && $request->has('mercado_pago') == 0 && $request->has('paytm') == 0) {
    ...
    public function statement($type = '', $request = null) {
        ...
        try {
            if ((isset($request->provider_id) && $request->provider_id != null) ||
				...
				$canceladas = UserRequests::with('payment')->orderBy('id', 'desc');
				
				$revenue = UserRequestPayment::select(\DB::raw(
                    'SUM(fixed) as overall, SUM(distance) as km, SUM(commision) as commission'
                ));
				
				$cancell = UserRequests::where('status', 'COMPLETED');
                $cancel_rides = UserRequests::where('status', 'COMPLETED');
            }
            ...
            if ($type == 'today') {
                ...
                if ($request->from_date == $request->to_date) {
                   ...
                   //TODO MARCIO-HAMASAKI - Alterações PIC_PAY
                    ...
                    }elseif($request->payment_mode && $request->payment_mode == 'pic_pay'){
                        $rides->where('payment_mode', 'PIC_PAY');
                    }elseif($request->payment_mode && $request->payment_mode == 'debit_machine'){
                        $rides->where('payment_mode', 'DEBIT_MACHINE');
                    }elseif($request->payment_mode && $request->payment_mode == 'mercado_pago'){
                        $rides->where('payment_mode', 'M_P');
                    }
            ...
            return view('admin.providers.statement', compact('rides','rides1', 'cancel_rides', 'Provider','canceladas', 'revenue1','revenue', 'pagination', 'dates', 'id', 'statement_for'))
	  ...
	  public function statement2( $request = null) {
		  
    $totalRides = UserRequests::has('user')->get()->count();
	$revenue1 = UserRequests::select(\DB::raw('SUM(status ) as overall' ));
				

            $revenue1 = $revenue1->get();

            
            return view('admin.providers.statement2', compact(  'revenue1','totalRides','user_cancelled'))
                            ->with('page', $page)->with('listname', $listname);
	...
	//transferencia carteira motorista
